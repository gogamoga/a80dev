...
#define A80_HUMBVGA_DEBUG

#ifdef A80_HUMBVGA_DEBUG
#define a80_humbvga_dbg(x...)	printk(x)
#else
#define a80_humbvga_dbg(x...)
#endif
...
//hanbiao addd for hub
static struct gpio_config hub_io;
/*
...
static int a80_humbvga_fetch_sysconfig_para(void)
{
	int ret = -1;
	int device_used = -1;
	script_item_u	val;
...
//hanbiao add for gps end
 ...
 //hanbiao add for hub start
 /* fetch reset/power/standby/flash/af io issue */
		type = script_get_item("vga_para","hub_en", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		      pr_err("type err hub_en \n");
			goto script_get_err;
		} else {
		  hub_io.gpio=val.gpio.gpio;
		  hub_io.mul_sel=val.gpio.mul_sel;
		}

//hanbiao add for hub end

		ret = 0;
		
	} else {
		pr_err("%s: a80_humbvga_unused. \n",  __func__);
		ret = -1;
	}
...
script_get_err:
	a80_humbvga_dbg("=========script_get_err============\n");
	return ret;
}
...
static int a80_humbvga_probe(struct i2c_client *client,
...
static int a80_humbvga_remove(struct i2c_client *client)
{
...
static int __init a80_humbvga_init(void)
{
	int ret = 0;
	printk("======%s=========. \n", __func__);
	
	if(!a80_humbvga_fetch_sysconfig_para())
	{
		vga_gpio_write(&enable_io, 1);	
		
		//hanbiao add for GPS
		vga_gpio_write(&gps_io, 1);	

		//hanbiao add for hub
		vga_gpio_write(&hub_io, 1);
...
static void __exit a80_humbvga_exit(void)
{
	//i2c_del_driver(&a80_humbvga_driver);
}

MODULE_AUTHOR("Albert Zhang <xu.zhang@bosch-sensortec.com>");
MODULE_DESCRIPTION("a80_humbvga driver");
MODULE_LICENSE("GPL");

module_init(a80_humbvga_init);
module_exit(a80_humbvga_exit);
