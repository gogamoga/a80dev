...
#define POWEROFF_MAX_DELAY        200

struct delayed_work sunxi_led_work;
...
static void gpio_led_work(struct work_struct *work)
{
   ...
   static int count = 0;
	...
	if(i<3)
	{
		gpio_leds[i].oldstate = !gpio_leds[i].oldstate;
		gpio_direction_output(gpio_leds[i].gpio, gpio_leds[i].oldstate);
		i++;
	}
	else
	{
	...
	if(count < 100)
	{
		 schedule_delayed_work(&sunxi_led_work, msecs_to_jiffies(POWEROFF_MAX_DELAY));
	}
	else
	{
		//hanbiao add for test.
		for(i=0; i<3; i++)
		{
			gpio_direction_output(gpio_leds[i].gpio, LEDS_GPIO_DEFSTATE_OFF);

			gpio_leds[i].oldstate =0;
		}
	}
...
//	}
      ...
      INIT_DELAYED_WORK(&sunxi_led_work, gpio_led_work);
	schedule_delayed_work(&sunxi_led_work, msecs_to_jiffies(POWEROFF_MAX_DELAY));
...
//hanbiao add for test.
	...
	for(i=0; i<3; i++)
	{
		ret = gpio_direction_output(gpio_leds[i].gpio, LEDS_GPIO_DEFSTATE_ON);

		gpio_leds[i].oldstate =1;

		if (ret < 0)
		 printk("======BILL error errorerrorerror=============\n");
	}
#endif

	////printk("======BILL LED okokokok=============\n");
